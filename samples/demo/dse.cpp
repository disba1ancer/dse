...
int main(int argc, char* argv[])
{
    ...
    loop.Periodic(100000, timerIO).detach();
    ...
    BasicBitmapLoader texture(context, u8"assets/textures/wall_test.bmp");
    BasicBitmapLoader mapTex(context, u8"assets/textures/earth.bmp");
    BasicBitmapLoader mapNorm(context, u8"assets/textures/earthnormalfull.bmp", true);
    ...
    auto mmoveCon = window.Register<MouseMove>(onMouseMove);
    // auto onResize = [](int, int, WindowShowCommand) {
    //     mainThread.yieldTasks();
    // };
    // auto sizeCon = window.Register<Resize>(onResize);
    ...
    loop.Periodic(10000, doStep).detach();
    return loop.Run();
